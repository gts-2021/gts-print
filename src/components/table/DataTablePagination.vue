	...
	<div class="gts-print-pagination-container">
		...
		<div class="gts-print-pagination-page-length-container">
			...
			<div>
				...
				<span class="gts-print-pagination-icon" @click="togglePagination"><v-icon>{{ "mdi-chevron-down"
				}}</v-icon></span>
			...
			<div v-if="pageLengthDisplayed" class="gts-print-pagination-page-length-menu">
				...
				<span v-for="page in effectivePageLengths" :key="page"
					class="gts-print-pagination-page-length-menu-item" @click="handlePageLengthChange(page)"> {{ page }}
				</span>
		...
		<div class="gts-print-pagination-steps">
			...
			<span class="gts-print-pagination-icon" @click="previousPage" :class="{ 'disabled': isFirstPage }">
				<v-icon>{{ "mdi-chevron-left" }}</v-icon>
			</span>

			<span class="gts-print-pagination-icon" @click="nextPage" :class="{ 'disabled': isLastPage }">
...
export default {
	...
	emits: ['changePage', 'lengthPageChanged', 'nextPage', 'previousPage'],
	...
	data() {
		const lengths = this.pageLengths || this.paginationConfig?.pageLengths || this.paginationConfig?.pageLength || [10, 20, 50, 100];
		const initialLength = typeof this.paginationConfig?.pageLength === 'number'
			? this.paginationConfig.pageLength
			: (Array.isArray(lengths) ? lengths[0] : 10);
		const initialPage = this.pageNumber || this.paginationConfig?.pageNumber || this.paginationConfig?.currentPage || 1;
		return {
			selectedPageLength: initialLength,
	...
	watch: {
		...
		'paginationConfig.pageNumber'(val) {
			if (val !== undefined && val !== this.currentPage) {
				this.currentPage = val;
			}
		},
		'paginationConfig.pageLength'(val) {
			if (typeof val === 'number' && val !== this.selectedPageLength) {
				this.selectedPageLength = val;
			}
		}
	},
	...
	computed: {
		...
		effectivePageLengths() {
			if (Array.isArray(this.pageLengths)) return this.pageLengths;
			if (Array.isArray(this.paginationConfig?.pageLengths)) return this.paginationConfig.pageLengths;
			if (Array.isArray(this.paginationConfig?.pageLength)) return this.paginationConfig.pageLength;
			return [10, 20, 50, 100];
		},
		...
		totalPages() {
			if (this.paginationConfig?.totalPages !== undefined) return this.paginationConfig.totalPages;
			if (this.effectiveTotalRecords === 0) return 1;
			return Math.max(1, Math.ceil(this.effectiveTotalRecords / this.selectedPageLength));
		},
	...
	methods: {
		...
		handlePageLengthChange(page) {
			...
			this.pageLengthDisplayed = false;
			this.currentPage = 1;
